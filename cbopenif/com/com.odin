...
ConnectCom :: proc() -> (ok: bool)
{
    ...
    ok = CreateComInstance(&clsid, &iid, cast(^rawptr)&cbopenif)
    if !ok {
        win.CoUninitialize()
        cbopenif = nil
        return
    ...
    ok = CreateComInstance(&clsid, &iid, cast(^rawptr)&objectfactory)
    if !ok {
        win.CoUninitialize()
        objectfactory = nil
        return
...
ComConnected :: proc() -> (ok: bool)
{
    if (cbopenif != nil) & (objectfactory != nil) do return true
    return false
}

DisconnectCom :: proc()  -> (ok: bool)
{
...
InitializeCom :: proc() -> (ok: bool)
{
    hr := win.CoInitializeEx(nil, win.COINIT.APARTMENTTHREADED)
    if ComFailed(hr) do return false
    return true
}

CreateComInstance :: proc(clsid: win.REFCLSID, iid: win.REFIID, vtable: ^win.LPVOID) -> (ok: bool)
{
...
ComFailed :: proc(hr: HResult) -> (failed: bool) {
...
GetComDispID :: proc(this: ^IUnknownIF, name: string) -> (id: i32, ok: bool)
{
...
InvokeCom :: proc(this: ^IUnknownIF, dispid: i32, args: []Variant, result: ^Variant, wflags := DISPATCH_METHOD) -> (hr: HResult) {
    ...
    if excep.bstrSource != nil      do FreeBstr(excep.bstrSource)
    if excep.bstrDescription != nil do FreeBstr(excep.bstrDescription)
    if excep.bstrHelpFile != nil    do FreeBstr(excep.bstrHelpFile)

    return hr
}

// Invoke by name (GetIDsOfNames + Invoke)
InvokeComName :: proc(this: ^IUnknownIF, name: string, args: []Variant, result: ^Variant = nil, wflags := DISPATCH_METHOD) -> (ok: bool)
{
    hr := HResult(-2147352573) // DISP_E_MEMBERNOTFOUND-ish 0x80020003

    dispid, found := GetComDispID(this, name)
    if !found do return
    
    hr = InvokeCom(this, dispid, args, result, wflags)
