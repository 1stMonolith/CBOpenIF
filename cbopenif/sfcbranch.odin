package cbopenif

SFCBranch :: distinct rawptr
SFCElements :: distinct rawptr   // collection returned by the property

SFCBranchIF :: struct #raw_union {
    #subtype unnknown_and_dispatch: UnknownAndDispatchIF,
    using vtable: ^SFCBranchVTable,
}

SFCBranchVTable :: struct {
    using unnknown_and_dispatch_vtable: UnknownAndDispatchVTable,
    PriorityGet: proc "system" (this: ^SFCBranchIF, SFCPriority: ^SFCPriorityValue) -> HResult,
    PriorityPut: proc "system" (this: ^SFCBranchIF, SFCPriority: SFCPriorityValue) -> HResult,
    ElementsGet: proc "system" (this: ^SFCBranchIF, SFCElements: ^SFCElements) -> HResult,
    Missing10:   proc "system" (this: ^SFCBranchIF) -> HResult,
    ElementsPut: proc "system" (this: ^SFCBranchIF, SFCElements: SFCElements) -> HResult,
}

sfcbranch_priority :: proc {
    sfcbranch_priority_,
    sfcbranch_priority_set,
}

@(private)
sfcbranch_priority_ :: proc(sfcbranch: SFCBranch) -> (priority: SFCPriorityValue, ok: bool) {
    priority = .Default
    ok = false

    if sfcbranch == nil do return
    if !connected() do return

    hr := (^SFCBranchIF)(sfcbranch)->PriorityGet(&priority)
    if failed(hr) do return

    return priority, true
}

@(private)
sfcbranch_priority_set :: proc(sfcbranch: SFCBranch, priority: SFCPriorityValue) -> (ok: bool) {
    ok = false

    if sfcbranch == nil do return
    if !connected() do return

    hr := (^SFCBranchIF)(sfcbranch)->PriorityPut(priority)
    if failed(hr) do return

    return true
}

sfcbranch_elements :: proc {
    sfcbranch_elements_,
    sfcbranch_elements_set,
}

@(private)
sfcbranch_elements_ :: proc(sfcbranch: SFCBranch) -> (elements: SFCElements, ok: bool) {
    elements = nil
    ok = false

    if sfcbranch == nil do return
    if !connected() do return

    hr := (^SFCBranchIF)(sfcbranch)->ElementsGet(&elements)
    if failed(hr) do return

    return elements, true
}

@(private)
sfcbranch_elements_set :: proc(sfcbranch: SFCBranch, elements: SFCElements) -> (ok: bool) {
    ok = false

    if sfcbranch == nil do return
    if !connected() do return

    hr := (^SFCBranchIF)(sfcbranch)->ElementsPut(elements)
    if failed(hr) do return

    return true
}

sfcbranch_release :: proc(sfcbranch: SFCBranch) {
    if sfcbranch != nil {
        (^SFCBranchIF)(sfcbranch)->Release()
    }
}