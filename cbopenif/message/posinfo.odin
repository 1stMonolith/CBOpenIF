...
posinfo_element_name_set :: proc(posinfo: PosInfo, element_name: string) -> (ok: bool) {
    ...
    bs := com.from_string(element_name)
    defer com.bstr_free(bs)
...
posinfo_message_type :: proc {
    posinfo_message_type_get,
    posinfo_message_type_set,
}

posinfo_message_type_get :: proc(posinfo: PosInfo) -> (message_type: type.MessageType, ok: bool) {
    if posinfo == nil do return
    if !controlbuilder.controlbuilder_connected() do return
    ...
    return type.MessageType(mt), true
}

posinfo_message_type_set :: proc(posinfo: PosInfo, message_type: type.MessageType) -> (ok: bool) {
    if posinfo == nil do return
    if !controlbuilder.controlbuilder_connected() do return
